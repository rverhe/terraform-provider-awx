...
```sh
mkdir -p ~/.terraform.d/plugins/github.com/rverhe/awx/0.1/linux_amd64/terraform-provider-awx
find ./dist/terraform-provider-awx_linux_amd64/* -name 'terraform-provider-awx*' -print0 | xargs -0 -I {} mv {} ~/.terraform.d/plugins/github.com/rverhe/awx/0.1/linux_amd64/terraform-provider-awx
```
...
```sh
goreleaser build --snapshot --rm-dist \
    && mkdir -p ~/.terraform.d/plugins/github.com/rverhe/awx/0.1/linux_amd64/ \
    && find ./dist/terraform-provider-awx_linux_amd64/* -name 'terraform-provider-awx*' -print0 | xargs -0 -I {} mv {} ~/.terraform.d/plugins/github.com/rverhe/awx/0.1/linux_amd64/terraform-provider-awx \
    && go test ./test -count=1
```
